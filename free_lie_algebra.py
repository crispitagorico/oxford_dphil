...
def concatenate(a,b):
    """concatenation product on the tensor space"""
...
class HallBasis:
    def __init__(self, d, m, lessExpression=lessExpressionLyndon):
        ...
        #                    out[-1].append((x,y))
        #self.data=out
        if lessExpression==lessExpressionLyndon:
            s = iisignature.prepare(d, m)
        elif lessExpression==lessExpressionStandardHall:
            s = iisignature.prepare(d, m, 'DH')
        list_hall = []
        lev_track = 1
        for lev in range(1,m+1):
            list_hall_level = []
            for k in iisignature.basis(s):
                fol = k.replace('[','').replace(']','').replace(',','')
                if len(fol)==lev_track:
                    kk = k
                    for let in range(1, d+1):
                        kk = kk.replace(str(let), '({},)'.format(let))
                    list_hall_level.append(make_tuple(kk.replace('[', '(').replace(']', ')')))
            list_hall.append(list_hall_level)
            lev_track+=1
        self.data=list_hall
